<?php
	...
	*****************************************************************************************************************/
	function find_patients_last_n_videos_twoDimensional($patiendID,$startDate,$endDate,$n,$ONLY_FALLS){
		...
		$link = mysql_connect($GLOBALS['DB_HOSTNAME'], $GLOBALS['DB_USERNAME'], $GLOBALS['DB_PASSWORD'])
		or die("Unable to connect to MySQL");
		//echo "Connected to MySQL<br>";

		//--------------------------------------------------------------------
		$queryString = "SELECT DISTINCT v.UserID, v.VideoDateTimeString, concat(v.BaseVideoPath, v.VideoPath) as VideoPath, CONCAT(v.VideoSize DIV 1048576,' MB') as VideoSize ,(CASE WHEN ISNULL(f.VideoDateTimeString) THEN 0 ELSE 1 END) as hasFall,1 
						FROM kinectrewind.tblkinectvideos as v LEFT OUTER JOIN kinectrewind.tblkinectfalls as f
		...
		//--------------------------------------------------------------------
// 		$queryString = "SELECT DISTINCT v.UserID, v.VideoDateTimeString, concat(v.BaseVideoPath, v.VideoPath) as VideoPath, CONCAT(v.VideoSize DIV 1048576,' MB') as VideoSize ,(CASE WHEN ISNULL(f.VideoDateTimeString) THEN 0 ELSE 1 END) as hasFall,count(1) as FallCounts
// 						FROM kinectrewind.tblkinectvideos as v LEFT OUTER JOIN kinectrewind.tblkinectfalls as f
// 						ON (v.UserId=f.UserID) AND(v.BaseVideoPath=f.BaseVideoPath) AND(TIMESTAMPDIFF(SECOND,v.VideoDateTimeString,f.VideoDateTimeString) between 0 and 15*60)
// 							AND (v.userid=".$patiendID.") AND (v.BaseVideoPath='".$GLOBALS['BASE_VIDEO_PATH']."')
// 						WHERE (v.userid=".$patiendID.") AND (v.BaseVideoPath='".$GLOBALS['BASE_VIDEO_PATH']."') 
...
// 		$queryString = $queryString." Group by v.UserID, v.VideoDateTimeString,v.BaseVideoPath ";
// 		$queryString = $queryString." ORDER BY v.VideoDateTimeString DESC ";
		
		//--------------------------------------------------------------------
// 		$queryString = "SELECT UserID, VideoDateTimeString, concat(BaseVideoPath, VideoPath) as VideoPath, CONCAT(VideoSize DIV 1048576,' MB') as VideoSize ,VideoUniqIndex 
// 										FROM kinectrewind.tblkinectvideos
// 										WHERE (userid=".$patiendID.")
// 											AND (VideoDateTimeString>='".$startDate."')
// 											AND (VideoDateTimeString<='".$endDate."')
// 											AND (VideoSize>=".$GLOBALS['MIN_VIDEO_SIZE'].")
// 											AND (BaseVideoPath='".$GLOBALS['BASE_VIDEO_PATH']."')
// 										ORDER BY VideoDateTimeString DESC ";
		...
		$MySQLResult = null;
		try{
			$MySQLResult = mysql_query($queryString);
	...
	function find_falls_within_video($patiendID,$video_date_time,$n){
		...
		$queryString = "SELECT VideoDateTimeString as fallTimeStamp
						FROM kinectrewind.tblkinectfalls
